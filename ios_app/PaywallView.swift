...
struct PaywallView: View {
    ...
    @State private var selectedPlan: String = "com.kokicoder.snapmath.pro.annual"
    ...
    private var heroGlow: some View {
        ZStack {
            ...
            Ellipse().fill(Color(hex: "10B981").opacity(0.07)).frame(width: 220, height: 180)
            ...
            ZStack {
                ...
                Image(systemName: "function")
                    ...
                    .foregroundStyle(LinearGradient(
                        colors: [Color.appAccent, Color(hex: "10B981")],
    ...
    private let features: [(icon: String, color: Color, title: String, detail: String)] = [
        ("infinity",           Color(hex: "A78BFA"), "Unlimited solves",        "No monthly limits"),
        ("camera.fill",        Color(hex: "34D399"), "Camera & photo library",  "Snap any math problem"),
        ("list.number",        Color(hex: "F59E0B"), "Step-by-step solutions",  "Full working shown"),
        ("clock.fill",         Color(hex: "60A5FA"), "Complete history",        "All solved problems saved"),
        ("sparkles",           Color(hex: "34D399"), "AI-powered by Claude",    "Most accurate AI model"),
        ("nosign",             Color(hex: "E74C3C"), "Zero ads",                "Pure math focus"),
    ]
        ...
        VStack(spacing: 10) {
            if let p = store.proAnnual {
                PlanCard(title: "Annual", displayPrice: p.displayPrice, period: "/ year",
                         badge: "Best Value · Save 41%", badgeColor: Color(hex: "2ECC71"),
            ...
            Button { Task { await startPurchase() } } label: {
                ZStack {
                    if isPurchasing { ProgressView().tint(.white) }
                        ...
                        HStack(spacing: 8) {
                            Text(selectedPlan.contains("annual") ? "Start Free Trial" : "Get Pro Access")
